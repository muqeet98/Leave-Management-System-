...
class EmpAttandanceE extends React.Component {
state = {
    data: []
}
renderItem = ({ item })=> {
  ...
  return (
    <View style={styles.listItem}>
      ...
      <Text style={{marginLeft: ('5%')}}>{item.attendance_id}</Text>
      <View style={{ alignItems: "center", flex: 1 }}>
        <Text style={{ fontWeight: "bold" }}>{item.date}</Text>
      </View>
      <View>
      <Text style={{ fontWeight: "bold", marginRight:('5%') }}>{item.status}</Text>
      </View>   
    </View>
  );
}
